...
export function TrafficDonutChart() {
  ...
  return (
    <div className="bg-white dark:bg-slate-800 rounded-xl p-5 border border-slate-200/80 dark:border-slate-700/50">
      <div className="mb-4">
        <h3 className="text-sm font-semibold text-slate-900 dark:text-slate-100">Traffic Sources</h3>
        <p className="text-xs text-slate-400 dark:text-slate-500 mt-0.5">Where visitors come from</p>
      </div>
      ...
      <div className="relative">
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
          ...
          <span className="text-[10px] text-slate-400 dark:text-slate-500 uppercase tracking-wider">Coverage</span>
        </div>
      </div>
      ...
      <div className="mt-4 space-y-2.5">
        {trafficSources.map((s) => (
          <div key={s.name} className="flex items-center gap-2.5">
            ...
            <span className="text-xs text-slate-600 dark:text-slate-300 flex-1 truncate">{s.name}</span>
            <div className="h-1 w-14 bg-slate-100 dark:bg-slate-700 rounded-full overflow-hidden">
              <div className="h-full rounded-full" style={{ width: `${s.value}%`, background: s.color }} />
            </div>
            <span className="text-xs font-semibold text-slate-700 dark:text-slate-200 w-7 text-right">{s.value}%</span>
          </div>
        ))}
      </div>
    </div>
